...
interface MVPPrioritizationProps {
  epicsAndTasks: any;
  onComplete: (prioritized: any) => void;
}

const MVPPrioritization: React.FC<MVPPrioritizationProps> = ({ epicsAndTasks, onComplete }) => {
  const [prioritizedFeatures, setPrioritizedFeatures] = useState<any>(null);
  ...
  useEffect(() => {
    const fetchPrioritization = async () => {
      setLoading(true);
      try {
        const features = epicsAndTasks.epics.map((epic: any) => epic.title);
      ...
      {prioritizedFeatures && (
        <form onSubmit={handleSubmit}>
          <div className="mb-4">
            <h4>MVP Features</h4>
            <ul>
              {prioritizedFeatures.mvpFeatures.map((feature: any) => (
          ...
          <div className="mb-4">
            ...
            <ul>
              {prioritizedFeatures.futureFeatures.map((feature: any) => (
                <li key={feature.id}>{feature.title}</li>
              ))}
            </ul>
          </div>
